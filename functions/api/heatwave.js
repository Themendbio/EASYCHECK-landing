import { REG_TO_SIDO } from '../../components/ui/kmaRegionMap.js';

// 기상청 폭염 영향예보(발표현황)를 17개 시·도별 최대 위험수준으로 롤업해 반환.
// Cloudflare Pages Function (정적 export + 단일 API). 응답: { asOf, levels, active }
...
export async function onRequestGet({ env }) {
    ...
    if (!key) {
        return json({ error: 'KMA_API_KEY not configured' }, 500);
    }
    ...
    try {
        ...
        // 응답은 EUC-KR이지만 파싱 대상(REG_ID·ILVL·날짜)은 전부 ASCII라 utf-8로 디코딩.
        // (Cloudflare Workers 런타임은 euc-kr 미지원 — 결과는 euc-kr 디코딩과 동일함을 검증)
        const buf = await res.arrayBuffer();
        text = new TextDecoder().decode(buf);
    } catch (e) {
        return json({ error: 'KMA fetch failed', detail: String(e) }, 502);
    }
    ...
    return json({ asOf: latestFc || null, levels, active }, 200, 1800);
}
